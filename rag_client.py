...
# Fetch / crawler
USER_AGENT        = os.getenv("CRAWLER_UA", "alma-bot/1.0 (+crawler)")
TIMEOUT_FETCH_S   = int(os.getenv("FETCH_TIMEOUT_S", "20"))
DEFAULT_MAX_PAGES = int(os.getenv("CRAWL_MAX_PAGES", "800"))       # generoso
DEFAULT_MAX_DEPTH = int(os.getenv("CRAWL_MAX_DEPTH", "4"))         # generoso
DEFAULT_DEADLINE  = int(os.getenv("RAG_DEADLINE_S", "180"))        # generoso

# Chunking / safety
UPSERT_BATCH      = int(os.getenv("UPSERT_BATCH", "64"))
MAX_RAW_CHARS     = int(os.getenv("MAX_RAW_CHARS", "50000"))       # pré-truncate por página
CHUNK_TOKENS      = int(os.getenv("CHUNK_TOKENS", "450"))
MAX_CHUNKS_PAGE   = int(os.getenv("MAX_CHUNKS_PAGE", "200"))       # cap por página

# Sitemap
SITEMAP_MAX_URLS  = int(os.getenv("SITEMAP_MAX_URLS", "1000"))     # generoso

# Dimensões por modelo (OpenAI 1536D)
MODEL_DIMS = {
...
def ensure_collection(dim: int = VECTOR_SIZE):
    """
    Garante que a collection existe com a dimensão correta.
    Se existir com dimensão diferente, lança erro explícito (para evitares 400 nos upserts).
    """
    try:
        ...
        if cfg and getattr(cfg, "vectors_config", None):
            ...
            # qdrant-client APIs variam consoante versão
...
DENY_CONTAINS = [
    ...
    "sessionid=", "phpsessid", "sid=", "fbclid=", "gclid="
]
...
def _chunk_text(text: str, max_tokens: int = CHUNK_TOKENS, max_chunks: int = MAX_CHUNKS_PAGE) -> List[str]:
    """
    Split simples por frases, com cap de nº de chunks.
    Faz fallback para 'split por palavras' se houver uma frase gigante.
    """
    parts: List[str] = []
    buf: List[str] = []
    count = 0

    # primeiro corte bruto por segurança (alguns sites têm dumps gigantes num só nó)
        ...
        if toks > max_tokens:
            ...
            for i in range(0, len(words), max_tokens):
                ...
                if len(parts) >= max_chunks:
                    return parts
            # volta ao loop principal (esta frase já foi tratada)
            buf, count = [], 0
            continue
    ...
    if buf and len(parts) < max_chunks:
        parts.append(". ".join(buf))
    return parts
...
def _ingest(namespace: str, url: str, title: str, full_text: str) -> int:
    chunks = _chunk_text(full_text, max_tokens=CHUNK_TOKENS, max_chunks=MAX_CHUNKS_PAGE)
    if not chunks:
        return 0
...
def ingest_text(title: str, text: str, namespace: str = "default"):
    ...
    return {"ok": True, "count": count, "namespace": namespace}

def ingest_url(page_url: str, namespace: str = "default", deadline_s: int = DEFAULT_DEADLINE):
    u = _clean_url(page_url)
    if not _url_allowed(u):
        return {"ok": False, "error": "url_blocked", "url": u, "namespace": namespace}
    try:
        r = requests.get(u, timeout=TIMEOUT_FETCH_S, headers={"User-Agent": USER_AGENT})
        r.raise_for_status()
    except Exception as e:
        return {"ok": False, "error": f"fetch_failed: {e}", "url": u, "namespace": namespace}
    soup = BeautifulSoup(r.text, "html.parser")
    title = (soup.title.string if soup.title else u).strip()
    text = soup.get_text(" ", strip=True)
    count = _ingest(namespace, u, title, text)
    return {"ok": True, "url": u, "count": count, "namespace": namespace}
...
def ingest_pdf_url(pdf_url: str, title: Optional[str] = None, namespace: str = "default"):
    ...
    try:
        ...
        return {"ok": False, "error": f"fetch_pdf_failed: {e}", "url": pdf_url, "namespace": namespace}
    doc = fitz.open("pdf", r.content)
    full = " ".join(page.get_text() for page in doc)
    count = _ingest(namespace, pdf_url, title or pdf_url, full)
    return {"ok": True, "url": pdf_url, "count": count, "namespace": namespace}

def ingest_sitemap(sitemap_url: str, namespace: str = "default", max_pages: int = SITEMAP_MAX_URLS, deadline_s: int = DEFAULT_DEADLINE):
    try:
        r = requests.get(sitemap_url, timeout=TIMEOUT_FETCH_S, headers={"User-Agent": USER_AGENT})
        r.raise_for_status()
    except Exception as e:
        return {"ok": False, "error": f"fetch_sitemap_failed: {e}", "namespace": namespace}

    # usar parser XML para evitar warnings e ser mais robusto
    ...
    for loc in locs[:max_pages]:
        ...
        if res.get("ok"):
            ...
            fail += 1
            failed_urls.append((loc, res.get("error", "unknown")))
