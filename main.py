...
# =========================
# HOTFIX /ask (memória + variantes completas)
...
# Cache leve em memória (por processo)
...
def _get_recent_dialog_pairs(user_id: str, max_pairs: int = 10) -> List[Tuple[str, str]]:
    """
    Tenta ler histórico do teu "local dialog" (se existir no projeto).
    Devolve lista [(user, alma), ...] dos últimos pares.
    """
    # Tens local_append_dialog(user_id, q, a). Muitos projetos também têm local_get_dialog.
    if "local_get_dialog" in globals() and callable(globals()["local_get_dialog"]):
        try:
            rows = globals()["local_get_dialog"](user_id) or []
            # rows pode ser lista de dicts/tuples; normalizamos
            pairs = []
            for r in rows[-max_pairs:]:
                if isinstance(r, dict):
                    q = r.get("q") or r.get("question") or r.get("user") or ""
                    a = r.get("a") or r.get("answer") or r.get("alma") or ""
...
def _extract_cart_from_text(pairs: List[Tuple[str, str]]) -> List[Dict[str, str]]:
    """
    Extrai itens (SKU/variante/preço/url) de respostas anteriores da Alma.
    Isto dá ao modelo "memória operacional" de orçamento.
    """
    items = []
    # padrões tolerantes
    sku_re = re.compile(r"\bSKU[:\s]*([0-9A-Za-z\-\._]+)", re.I)
    price_re = re.compile(r"(?:preço(?:\s+unit[aá]rio)?|unit[aá]rio)[:\s]*([0-9][0-9\.,]*)\s*€", re.I)
    variant_re = re.compile(r"(?:variante|op[cç][aã]o)[:\s]*([^\n|•\-]+)", re.I)
    url_re = re.compile(r"(https?://[^\s\)]+)", re.I)

    for (_q, a) in pairs:
        if not a:
            continue
        skus = sku_re.findall(a)
        urls = url_re.findall(a)
        prices = price_re.findall(a)
        variants = variant_re.findall(a)

        # heurística: cria 1 item por SKU encontrado
        for idx, sku in enumerate(skus[:20]):
            item = {"sku": sku.strip()}
            if idx < len(prices):
                item["price_eur"] = prices[idx].strip()
            # variante: se houver, usa a primeira "boa"
            if variants:
                item["variant"] = variants[0].strip()
            # url: usa a primeira que pareça do interiorguider
            best_url = ""
            for u in urls:
                if "interiorguider.com" in u:
                    best_url = u
                    break
            if best_url:
                item["url"] = best_url
    ...
    # dedupe por sku+variant
    seen = set()
    out = []
    for it in items:
        key = (it.get("sku",""), it.get("variant",""))
        if key in seen:
            continue
        seen.add(key)
        out.append(it)
...
def _is_variants_question(q: str) -> bool:
    ql = (q or "").lower()
    return any(k in ql for k in [
        "todas as variantes", "todas as opções", "variantes", "opções", "opcoes",
        "quais são as variantes", "lista as variantes", "variantes disponíveis", "variantes disponiveis"
    ])

def _guess_product_name(q: str) -> Optional[str]:
    """
    Heurística simples: tenta apanhar um nome de produto a partir do texto.
    Para o teu caso funciona bem porque os pedidos são do tipo "zeal laser", "dublexo", etc.
    """
    t = (q or "").strip()
    if not t:
        return None
    # remove palavras de ação comuns
    t = re.sub(r"\b(faz|fazer|or[cç]amento|orcamento|junta|adiciona|acrescenta|mais|um|uma|de|do|da|dos|das|para)\b", " ", t, flags=re.I)
    t = re.sub(r"\s+", " ", t).strip()
    # se ficar curto, devolve None
    if len(t) < 3:
        return None
    # limita a 60 chars
    return t[:60]

def _open_catalog_db() -> Optional[sqlite3.Connection]:
    path = globals().get("CATALOG_DB_PATH") or globals().get("CATALOG_DB") or None
    if not path:
        # fallback conhecido pelo teu deploy
...
def _find_best_table(conn: sqlite3.Connection) -> Optional[str]:
    """
    Descobre automaticamente a tabela "catálogo" (a que tem colunas tipo nome/variante/preco/url).
    """
    try:
        tables = conn.execute(
        ...
        for r in tables:
            ...
            score = 0
            for must in ["nome", "variante", "preco"]:
                if must in cols_l:
                    score += 3
            for nice in ["url", "url_canonico", "ref", "sku", "marca"]:
                if nice in cols_l:
                    score += 1
            if score >= 6:
                candidates.append((score, name, cols))
        if not candidates:
            return None
...
def _fetch_all_variants(conn: sqlite3.Connection, product_hint: str, limit: int = 80) -> List[sqlite3.Row]:
    """
    Procura linhas do produto (por nome/ref/sku) e depois agrupa por url_canonico/url.
    """
    table = _find_best_table(conn)
    if not table:
        return []
    ...
    # escolhe colunas existentes
    col_nome = "nome" if "nome" in cols_l else None
    col_var = "variante" if "variante" in cols_l else None
    col_preco = "preco" if "preco" in cols_l else None
    col_ref = "ref" if "ref" in cols_l else ("sku" if "sku" in cols_l else None)
    col_url = "url" if "url" in cols_l else None
    col_urlc = "url_canonico" if "url_canonico" in cols_l else None
    col_marca = "marca" if "marca" in cols_l else None

    # query base (LIKE no nome; e também tenta ref/sku se o user escreveu parecido)
    wheres = []
    params = []
    hint = (product_hint or "").strip()
    like = f"%{hint}%"
    if col_nome:
        wheres.append(f"LOWER({col_nome}) LIKE LOWER(?)")
        params.append(like)
    if col_ref:
        wheres.append(f"LOWER({col_ref}) LIKE LOWER(?)")
        params.append(like)

    if not wheres:
        return []

    sql = f"SELECT * FROM {table} WHERE (" + " OR ".join(wheres) + f") LIMIT {limit}"
    rows = conn.execute(sql, params).fetchall()
    if not rows:
        return []

    # encontra key canónica: url_canonico se existir, senão url (mas limpa lixo tipo '#sku=...')
    def _canon_from_row(r):
        if col_urlc and r.get(col_urlc):
            return str(r.get(col_urlc)).strip()
        if col_url and r.get(col_url):
            u = str(r.get(col_url)).strip()
            # remove fragmentos típicos
            u = u.split("#")[0].split("?")[0]
            return u
        return ""

    # escolhe a canónica mais frequente
    keys = [_canon_from_row(r) for r in rows if _canon_from_row(r)]
    key = max(set(keys), key=keys.count) if keys else ""

    if not key:
        return rows

    # agora vai buscar TODAS as variantes desse canonical (sem LIMIT apertado)
    wh2 = []
    p2 = []
    if col_urlc:
        wh2.append(f"{col_urlc} = ?")
        p2.append(key)
    elif col_url:
        # compara por url sem fragment/query
        wh2.append(f"{col_url} LIKE ?")
        p2.append(key + "%")

    if not wh2:
        return rows

    sql2 = f"SELECT * FROM {table} WHERE " + " OR ".join(wh2) + " ORDER BY " + (col_var or col_nome or col_ref or "rowid")
    return conn.execute(sql2, p2).fetchall()

def _format_variants_answer(rows: List[sqlite3.Row]) -> str:
    if not rows:
        return "Não encontrei variantes no catálogo interno para esse produto."

    # tenta inferir campos
    keys = rows[0].keys()

    def getv(r, *names):
        for n in names:
            if n in keys and r.get(n) not in (None, ""):
                return str(r.get(n)).strip()
            # também tenta variantes em lowercase
            ln = n.lower()
            for k in keys:
                if k.lower() == ln and r.get(k) not in (None, ""):
                    return str(r.get(k)).strip()
        return ""

    # canonical link único (preferir url_canonico)
    canon = ""
    for r in rows:
        canon = getv(r, "url_canonico")
        if canon:
            break
    if not canon:
        # fallback: url limpo
    ...
    for r in rows:
        nm = getv(r, "nome")
        if nm:
            # reduz ruído "— Variante"
            base_name = nm.split("—")[0].strip()
            break

    lines = []
    if base_name:
        lines.append(f"Variantes de **{base_name}** no catálogo interno ({len(rows)}):")
    else:
        lines.append(f"Variantes no catálogo interno ({len(rows)}):")

    for r in rows:
        var = getv(r, "variante") or ""
        ref = getv(r, "ref", "sku") or ""
        preco = getv(r, "preco") or ""
        marca = getv(r, "marca") or ""
        bit = "- "
        if var:
            bit += f"**{var}**"
        else:
            nm = getv(r, "nome")
            bit += f"**{nm}**" if nm else "**(sem nome)**"
        if ref:
            bit += f" | SKU:{ref}"
        if preco:
            bit += f" | Preço: {preco}€"
    ...
    if canon:
        lines.append("")
        lines.append(f"ver produto: {canon}")

    return "\n".join(lines)

def _inject_memory_block(messages: List[Dict[str, str]], user_id: str) -> List[Dict[str, str]]:
    """
    Injeta contexto operacional do orçamento no system prompt sem mexer no resto.
    """
    pairs = _get_recent_dialog_pairs(user_id, max_pairs=12)
    cart_items = _extract_cart_from_text(pairs)
    if not pairs and not cart_items:
        return messages

    mem_lines = []
    if pairs:
        mem_lines.append("HISTÓRICO RECENTE (mesma sessão, para manter contexto):")
        for q, a in pairs[-8:]:
            mem_lines.append(f"- User: {q}")
            mem_lines.append(f"- Alma: {a}")
    if cart_items:
        mem_lines.append("")
        mem_lines.append("ITENS JÁ COTADOS (extraído do histórico; usar para 'juntar/adicionar' sem perder relação):")
        for it in cart_items[:12]:
            bits = []
            if it.get("sku"): bits.append(f"SKU:{it['sku']}")
            if it.get("variant"): bits.append(f"Variante:{it['variant']}")
            if it.get("price_eur"): bits.append(f"Preço:{it['price_eur']}€")
            if it.get("url"): bits.append(f"URL:{it['url']}")
            mem_lines.append("- " + " | ".join(bits))

    # injeta no topo como system (sem apagar o teu system original)
    mem_msg = {"role": "system", "content": "MEMÓRIA OPERACIONAL (hotfix)\n" + "\n".join(mem_lines)}
    return [mem_msg] + messages

def _remove_existing_ask_route():
    """
    Remove handler antigo do /ask (POST) para este hotfix assumir sem conflitos.
    """
    try:
        new_routes = []
...
_remove_existing_ask_route()

@app.post("/ask")
async def ask_hotfix(request: Request):
    data = await request.json()
    question = (data.get("question") or "").strip()
    user_id = (data.get("user_id") or "").strip() or "anon"
    namespace = (data.get("namespace") or "").strip() or None
    ...
    decided_top_k = _decide_top_k(question, req_top_k) if "_decide_top_k" in globals() else (req_top_k or 8)

    log.info(f"[HOTFIX /ask] user_id={user_id} ns={namespace or globals().get('DEFAULT_NAMESPACE','default')} top_k={decided_top_k} question={question!r}")

    if not question:
        return {"answer": "Coloca a tua pergunta em 'question'."}

    # --- 1) Se for pergunta de variantes/opções: bypass LLM e responde direto do catalog.db com todas ---
    if _is_variants_question(question):
        hint = _guess_product_name(question) or question
        conn = _open_catalog_db()
        if conn:
            try:
                rows = _fetch_all_variants(conn, hint, limit=120)
                ans = _format_variants_answer(rows)
                # guarda diálogo
                if "local_append_dialog" in globals() and callable(globals()["local_append_dialog"]):
                    globals()["local_append_dialog"](user_id, question, ans)
                return {"answer": ans, "rag": {"used": False, "top_k_effective": 0, "namespace": namespace}}
            except Exception:
                pass
            finally:
                try: conn.close()
    ...
    # --- 2) Fluxo normal: build_messages + injecção de memória operacional + LLM ---
    messages, new_facts, rag_used, rag_hits = build_messages(user_id, question, namespace)

    # injeta memória do próprio diálogo (para "junta/adiciona" não perder contexto)
    messages = _inject_memory_block(messages, user_id)
    ...
    # pós-processamento existente
    if "_postprocess_answer" in globals():
        answer = _postprocess_answer(answer, question, namespace, decided_top_k)

    # Mantém o teu comportamento de links, mas só se existir (não rebenta)
    if "_force_links_block" in globals():
        try:
            answer = _force_links_block(answer, rag_hits, min_links=3)
        except Exception:
            pass

    # guardar diálogo + mem0 (se existirem)
    if "local_append_dialog" in globals() and callable(globals()["local_append_dialog"]):
        globals()["local_append_dialog"](user_id, question, answer)

    if "_mem0_create" in globals() and callable(globals()["_mem0_create"]):
        try:
            globals()["_mem0_create"](content=f"User: {question}", user_id=user_id, metadata={"source": "alma-server", "type": "dialog"})
            globals()["_mem0_create"](content=f"Alma: {answer}", user_id=user_id, metadata={"source": "alma-server", "type": "dialog"})
        except Exception:
            pass
    ...
    return {
        ...
        "rag": {
            "used": rag_used,
            "top_k_default": globals().get("RAG_TOP_K_DEFAULT", None),
            "top_k_effective": decided_top_k,
            "namespace": namespace or globals().get("DEFAULT_NAMESPACE", "default")
        }
    }
