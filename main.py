...
# =====================================================================================
# HOTFIX SEGURO: Variantes completas por url_canonical (NUNCA quebra o /ask)
# Cola no fim do main.py, depois de tudo.
# =====================================================================================

import re
import sqlite3

def _hf_safe_norm(s: str) -> str:
    return re.sub(r"\s+", " ", (s or "").strip().lower())

def _hf_safe_extract_product_hint(text: str) -> str:
    t = (text or "").strip()
    # 1) entre aspas
    m = re.search(r"[“\"']([^”\"']{3,80})[”\"']", t)
    if m:
        return m.group(1).strip()
    # 2) duas+ palavras com maiúscula (ex.: Zeal Laser)
    m = re.search(r"\b([A-ZÁÂÃÀÉÊÍÓÔÕÚÇ][\wÁÂÃÀÉÊÍÓÔÕÚÇ]+(?:\s+[A-ZÁÂÃÀÉÊÍÓÔÕÚÇ][\wÁÂÃÀÉÊÍÓÔÕÚÇ]+){1,3})\b", t)
    return (m.group(1).strip() if m else "")

def _hf_safe_wants_variants(q: str) -> bool:
    ql = (q or "").lower()
    keys = ["variantes", "variante", "opções", "opcoes", "opcao", "cores", "tecidos", "acabamentos", "listar"]
    return any(k in ql for k in keys)

def _hf_safe_is_budget(q: str) -> bool:
    ql = (q or "").lower()
    keys = ["orçamento", "orcamento", "cotação", "cotacao", "proforma", "preço", "precos", "preços", "subtotal", "quantidade"]
    return any(k in ql for k in keys)

def _hf_safe_db_path() -> str:
    try:
...
def _hf_safe_has_column(con: sqlite3.Connection, table: str, col: str) -> bool:
    try:
...
def _hf_safe_pick_best_canonical(con: sqlite3.Connection, ns: str, hint: str) -> str | None:
    h = _hf_safe_norm(hint)
    if not h:
        return None
    try:
        rows = con.execute(
...
def _hf_safe_fetch_variants(con: sqlite3.Connection, ns: str, url_canonical: str):
    try:
        rows = con.execute(
            """
            SELECT ref, name, price, url, url_canonical, variant_attrs, summary, brand
            FROM catalog_items
            WHERE namespace = ?
              AND url_canonical = ?
            ORDER BY ref ASC
...
def _hf_safe_extract_variant_from_summary(summary: str) -> str:
    s = summary or ""
    m = re.search(r"(?:^|\n)\s*Variante:\s*(.+)", s, flags=re.I)
    return (m.group(1).strip() if m else "")

def _hf_safe_format_variants_md(rows) -> str:
    if not rows:
        return ""

    # tenta nome-base a partir do primeiro
    first_name = (rows[0][1] or "").strip()  # name
    title = first_name.split("—")[0].strip() if "—" in first_name else first_name

    out = []
    out.append(f"**Variantes no catálogo interno — {title}**")
    out.append("")
    out.append("| Variante | SKU | Preço (IVA incl.) | Link |")
    out.append("|---|---:|---:|---|")

    for r in rows:
        ref = (r[0] or "").strip() or "(sem dado)"         # ref
        name = (r[1] or "").strip()                        # name
        price = r[2]                                       # price
        url = (r[3] or "").strip()                         # url
        urlc = (r[4] or "").strip()                        # url_canonical
        vattrs = (r[5] or "").strip()                      # variant_attrs
        summary = (r[6] or "")                             # summary

        variante = vattrs or _hf_safe_extract_variant_from_summary(summary) or (name.split("—",1)[1].strip() if "—" in name else "(sem dado)")
        price_txt = f"{float(price):.2f} €" if price is not None and str(price).strip() != "" else "(sem dado)"
        link = urlc or url or "(sem dado)"

        out.append(f"| {variante} | {ref} | {price_txt} | [ver produto]({link}) |")

    out.append("")
    out.append("Preço com IVA incluído; portes não incluídos.")
    return "\n".join(out)

# --- Hook seguro no pós-processamento (nunca crasha) ---
if callable(globals().get("_postprocess_answer")):
    _hf_orig_postprocess_answer = globals()["_postprocess_answer"]

    def _postprocess_answer(answer, question, namespace, decided_top_k=None):  # type: ignore
        # 1) faz sempre o original
        out = _hf_orig_postprocess_answer(answer, question, namespace, decided_top_k)

        # 2) só tenta enriquecer se o pedido for variantes ou orçamento
        try:
            if not (_hf_safe_wants_variants(question) or _hf_safe_is_budget(question)):
                return out

            ns = (namespace or globals().get("DEFAULT_NAMESPACE") or "boasafra")
            db_path = _hf_safe_db_path()

            con = sqlite3.connect(db_path)
            try:
                # se não houver url_canonical, não faz nada
                if not _hf_safe_has_column(con, "catalog_items", "url_canonical"):
                    return out

                hint = _hf_safe_extract_product_hint(question) or _hf_safe_extract_product_hint(out)
                canonical = _hf_safe_pick_best_canonical(con, ns, hint)
                if not canonical:
                    return out

                rows = _hf_safe_fetch_variants(con, ns, canonical)
                if len(rows) < 2:
                    return out

                block = _hf_safe_format_variants_md(rows)
                return block or out
