...
# =============================================================================
# HOTFIX FINAL — Links dos itens no fim (via quotes.db snapshots)
# - NÃO usa RAG
# - Funciona mesmo com múltiplas rotas /ask (embrulha TODAS)
# - Só anexa se for orçamento e houver >=2 itens confirmados no quote
# =============================================================================
...
def _answer_looks_like_budget(text: str) -> bool:
    return bool(re.search(r"\bOrçamento\b|\bOrcamento\b", text or "", flags=re.I))

def _append_quote_links_block(answer: str, user_id: str, namespace: str) -> str:
    """
    Anexa um bloco no fim:
      Links dos itens:
      - [Nome — SKU](url)
    A fonte é quotes.db (url_snapshot/name_snapshot/ref).
    """
    if not answer:
        return answer

    # evita duplicar se já existe bloco
    if re.search(r"^\s*Links dos itens\s*:", answer, flags=re.I | re.M):
        return answer
    ...
    try:
        ns = (namespace or DEFAULT_NAMESPACE).strip()
        uid = (user_id or "anon").strip() or "anon"
        qid = _quote_has_open(uid, ns) or _quote_get_open(uid, ns)
        items = _quote_list_items(qid) or []
    except Exception:
        return answer

    # só interessa multi-itens
    if len(items) < 2:
        return answer
    ...
    for it in items:
        ...
        if not url:
            # fallback: tenta buscar do catálogo pelo ref
            try:
                cat = _catalog_get_by_ref(ns, ref) if ref else None
                url = (cat.get("url_canonical") or cat.get("url") or "").strip() if cat else ""
                if not name and cat:
        ...
        # normaliza relativo -> absoluto IG
        try:
            url = _canon_ig_url(url)
        except Exception:
            pass

        label = name or ref or "—"
    ...
    for label, url in links[:12]:
        block.append(f"- [{label}]({url})")

    return answer.rstrip() + "\n" + "\n".join(block)

# -----------------------------------------------------------------------------
# WRAP: embrulhar TODAS as rotas POST /ask (porque tens duplicadas)
# -----------------------------------------------------------------------------
def _wrap_all_ask_routes_for_links():
    try:
        wrapped = 0
        for r in getattr(app, "routes", []):
            ...
            # evita double-wrap
            if getattr(ep, "_hf_links_wrapped", False):
                continue
            ...
            async def _endpoint_wrapped(*args: Any, __ep=ep, **kwargs: Any):
                ...
                try:
                    ...
                    # tentar obter request obj (pydantic) para user_id/namespace
                    req_obj = args[0] if args else None
                    user_id = getattr(req_obj, "user_id", None) if req_obj else None
                    namespace = getattr(req_obj, "namespace", None) if req_obj else None

                    # fallback: tenta kwargs
                    if not user_id:
                        for v in kwargs.values():
                    ...
                    # aplica append
                    fixed = _append_quote_links_block(answer, user_id or "anon", (namespace or DEFAULT_NAMESPACE))
                    if fixed != answer:
                        resp["answer"] = fixed
                    return resp
                except Exception:
            ...
            _endpoint_wrapped._hf_links_wrapped = True
        ...
        try:
            log.warning(f"[hotfix-links] falhou: {e}")
        except Exception:
            pass
        return False

_wrap_all_ask_routes_for_links()
# =============================================================================
# FIM HOTFIX FINAL
# =============================================================================
