...
# ===================== HOTFIX — canonical_url + pós-import + output do chat =====================

def _hotfix_catalog_canonical_url_all():
    import sqlite3
    import logging
    from typing import Optional, List, Dict
    ...
    # ---------- (1) garantir coluna canonical_url ----------
    def _ensure_canonical_col():
        try:
            with _catalog_conn() as c:
                cols = [r["name"] for r in c.execute("PRAGMA table_info(catalog_items)").fetchall()]
                if "canonical_url" not in cols:
                    c.execute("ALTER TABLE catalog_items ADD COLUMN canonical_url TEXT")
                    c.commit()
            log.info("[hotfix] canonical_url: coluna OK")
            return True
        except Exception as e:
            log.warning(f"[hotfix] canonical_url: falhou a criar coluna: {e}")
            return False

    # ---------- (2) preencher canonical_url num namespace ----------
    def _fill_canonical_for_namespace(ns: str):
        """
        canonical_url = url sem fragment (#...), e se for relativo (/slug) prefixa https://IG_HOST
        """
        ns = (ns or DEFAULT_NAMESPACE or "").strip() or "default"
        try:
            with _catalog_conn() as c:
                # 1) base = url sem #...
                # 2) se começar por '/', prefixa domínio
                # 3) se canonical_url já existe, atualiza na mesma (queremos consistência)
                c.execute(
                    f"""
                    UPDATE catalog_items
                       SET canonical_url = (
                ...
                # prefixar domínio quando ficou relativo
                # (isto cobre casos em que url/canonical_url vem tipo "/zeal-laser")
                c.execute(
                    f"""
                    UPDATE catalog_items
                       SET canonical_url = 'https://{IG_HOST}' || canonical_url
                     WHERE namespace = ?
            ...
            log.info(f"[hotfix] canonical_url: preenchido para namespace={ns}")
            return True
        except Exception as e:
            log.warning(f"[hotfix] canonical_url: falhou a preencher ns={ns}: {e}")
            return False

    # ---------- (3) wrap do endpoint /catalog/import-csv ----------
    def _wrap_import_csv_to_fill_canonical():
        try:
            _orig = globals().get("catalog_import_csv", None)
            if not _orig or not callable(_orig):
                log.warning("[hotfix] canonical_url: não encontrei catalog_import_csv() em globals()")
                return False

            # wrapper: chama o teu import antigo -> depois preenche canonical_url nesse namespace
            ...
            async def _wrapped_catalog_import_csv(
                ...
                result = await _orig(file=file, namespace=namespace)
                try:
                    _ensure_canonical_col()
                    _fill_canonical_for_namespace((namespace or DEFAULT_NAMESPACE).strip())
                except Exception as e:
                    log.warning(f"[hotfix] canonical_url: pós-import falhou: {e}")
                return result

            # substituir endpoint na rota já existente
            replaced = False
            ...
            if not replaced:
                log.warning("[hotfix] canonical_url: não encontrei rota POST /catalog/import-csv para substituir")
                return False

            globals()["catalog_import_csv"] = _wrapped_catalog_import_csv
            log.info("[hotfix] canonical_url: /catalog/import-csv agora preenche canonical_url no fim")
            return True

        except Exception as e:
            log.warning(f"[hotfix] canonical_url: wrap import falhou: {e}")
            return False

    # ---------- (4) forçar build_catalog_block a usar canonical_url ----------
    # mantém a tua pesquisa “o mais parecido possível”, só muda o OUTPUT do link.
    def _install_catalog_blocks_using_canonical():
        try:
            # helpers tolerantes se existirem
            has_tolerant = ("_hf_tokens" in globals()) and ("_hf_like_pat" in globals())

            import re

            def _pick_link(row: dict) -> str:
            ...
            def build_catalog_block(question: str, namespace: Optional[str] = None, limit: int = 30) -> str:
                ...
                try:
                    with _catalog_conn() as c:
                        # match por SKU/ref se houver
                        if ref_toks:
                            q_marks = ",".join("?" * len(ref_toks))
                            cur = c.execute(
                                f"""SELECT name, ref, price, url, canonical_url, brand, variant_attrs
                                      FROM catalog_items
                                     WHERE namespace=? AND ref IN ({q_marks})
                                     ORDER BY updated_at DESC
                                     LIMIT ?""",
                        ...
                        # pesquisa por tokens (tolerante se existir)
                        if not rows:
                            if has_tolerant:
                                terms = globals()["_hf_tokens"](q_raw)
                                if terms:
                                    conds = []
                                    like_args = []
                                    for t in terms:
                                        pat = globals()["_hf_like_pat"](t)
                                        conds.append("(name LIKE ? ESCAPE '\\' OR summary LIKE ? ESCAPE '\\' OR ref LIKE ? ESCAPE '\\')")
                                        like_args.extend([pat, pat, pat])
                                    where = " AND ".join(conds)
                                    cur = c.execute(
                                ...
                                like = f"%{q_raw.lower()}%"
                                cur = c.execute(
                                    """SELECT name, ref, price, url, canonical_url, brand, variant_attrs
                                         FROM catalog_items
                                        WHERE namespace=? AND (LOWER(name) LIKE ? OR LOWER(summary) LIKE ? OR LOWER(ref) LIKE ?)
                                        ORDER BY updated_at DESC
                                        LIMIT ?""",
                    ...
                    log.warning(f"[hotfix] build_catalog_block falhou: {e}")
                    rows = []

                if not rows:
                    return ""

                lines = ["CATÁLOGO INTERNO (Link canónico; usar só estes dados para preços/orçamentos):"]
                seen = set()
                for r in rows[:limit]:
                    key = (r.get("ref"), r.get("url"))
                    if key in seen:
                    ...
                    ref  = (r.get("ref") or "(sem ref)").strip()
                    price = r.get("price")
                    price_txt = f"{price:.2f}€" if isinstance(price, (int, float)) else "(sem preço)"
                    ...
                    link = _pick_link(r)
                    lines.append(f"- {name_show} • SKU:{ref} • Preço:{price_txt} • Marca:{brand} • Link:{link}")
            ...
            def build_catalog_variants_block(question: str, namespace: Optional[str]) -> str:
                ...
                terms = []
                if has_tolerant:
                    terms = globals()["_hf_tokens"](question)
                if not terms and not question.strip():
                    return ""

                rows: List[Dict] = []
                try:
                    with _catalog_conn() as c:
                        if has_tolerant and terms:
                            conds = []
                            like_args = []
                            for t in terms:
                                pat = globals()["_hf_like_pat"](t)
                    ...
                    log.warning(f"[hotfix] build_catalog_variants_block falhou: {e}")
                    rows = []

                if not rows:
                    return ""

                # agrupar por canonical_url (ou por url sem #)
                groups: Dict[str, List[Dict]] = {}
                for r in rows:
                    parent = (r.get("canonical_url") or "").strip()
                    if not parent:
                        u = (r.get("url") or "").strip()
                        parent = u.split("#", 1)[0] if u else ""
                        if parent.startswith("/"):
                            parent = f"https://{IG_HOST}{parent}"
                    if parent:
                        groups.setdefault(parent, []).append(r)

                # manter só grupos com >=2 SKUs diferentes
                groups2 = {}
                for parent, lst in groups.items():
                    refs = { (it.get("ref") or "").strip() for it in lst if (it.get("ref") or "").strip() }
                    if len(refs) >= 2:
                        groups2[parent] = lst
                ...
                out = ["CATÁLOGO INTERNO — Variantes (Link canónico sempre o base):"]
                for parent, lst in list(groups2.items())[:20]:
                    lst_sorted = sorted(lst, key=lambda x: (x.get("variant_attrs") or x.get("name") or "", x.get("ref") or ""))
                    title = (lst_sorted[0].get("name") or "").split("—", 1)[0].strip() or parent
                    out.append(f"• Produto: {title} • Link:{parent}")
                    for r in lst_sorted[:40]:
                        ref  = (r.get("ref") or "(sem ref)").strip()
                        pr   = r.get("price")
                        pr_s = f"{pr:.2f}€" if isinstance(pr, (int, float)) else "(sem preço)"
                        va   = (r.get("variant_attrs") or "").strip()
                        label = va or (r.get("name") or "")
                        out.append(f"   - {label} | SKU:{ref} | {pr_s}")
                return "\n".join(out)

            globals()["build_catalog_block"] = build_catalog_block
            globals()["build_catalog_variants_block"] = build_catalog_variants_block
            log.info("[hotfix] canonical_url: build_catalog_block/variants agora usam Link canónico")
            return True

        except Exception as e:
            log.warning(f"[hotfix] canonical_url: falhou a instalar blocks: {e}")
            return False

    # executar tudo
    ok1 = _ensure_canonical_col()
    ok2 = _wrap_import_csv_to_fill_canonical()
    ok3 = _install_catalog_blocks_using_canonical()

    return ok1 and ok2 and ok3


_hotfix_catalog_canonical_url_all()
# ===================== /HOTFIX =================================================================
