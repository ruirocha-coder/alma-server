...
# =============================================================================
# HOTFIX — Import CSV
# Corrige: produtos SEM variantes não estavam a entrar no catálogo
# Regra correta:
#   - Item Type = Product
#   - Product URL obrigatório
#   - SKU continua exatamente igual
#   - NÃO cria URLs artificiais
# =============================================================================
...
async def catalog_import_csv(
    ...
    for row in reader:
        try:
            ...
            def _with_availability(text: str) -> str:
                if availability:
                    return (text + "\n" if text else "") + f"Disponibilidade: {availability}"
                return text
            ...
            if row_type == "Product":
                ...
                # ❗ produto invisível / oculto → ignorar
                if not base_url:
                    current_product = None
                    skipped += 1
                    items_out.append({
            ...
            # VARIANTE (SKU)
            # ------------------------------------------------------------------
            if row_type == "SKU" and current_product:

                variant_values = _variant_to_values_only(name_raw)
                vkey = values_to_variant_key(variant_values)
                ...
                url_variant = (
                    f"{current_product['url']}#sku={ref}"
                    if ref else current_product["url"]
                )

                _upsert_catalog_row(
                    ns=namespace,
                    name=(
                        f"{current_product['name']} — {variant_values}"
                        if variant_values else current_product["name"]
                    ),
                    summary=summary_v,
                    url=url_variant,
                    source="csv",
