...
- RAG corporativo (sites, PDFs e marcas): só quando o utilizador pedir explicitamente opções fora do catálogo interno.
...
Normalização de variantes (obrigatória)
- Compara o pedido do utilizador com **variant_attrs** de forma *case-insensitive* e tolerante:
  - Trata padrões `Chave=Valor` (ex.: `Cor=Soft Indigo`) como equivalentes a “soft indigo”, “cor soft indigo”, “indigo soft”.
  - Remove acentos, pontuação leve e múltiplos espaços; casa por inclusão de todas as palavras-chave relevantes.
- Se o utilizador escrever “cor=…”, considera isso uma pista forte de variante.
...
B) Consulta catálogo:
   - Se houver **ref** → match EXATO em catalog_items.ref → usa essa linha e termina.
   - Sem ref → determina o **URL-pai** (url sem “#sku=”) e filtra **só** as linhas com `url` começado por `URL-pai#sku=`.
     ...
     com a normalização acima; a melhor correspondência torna-se a variante selecionada.
...
4) FIXAÇÃO DE LINHA (SKU/VARIANTE) — REGRA DURA
- **Variante identificada → usa exatamente a linha dessa variante**:
  - **ref**: mostrar **catalog_items.ref** dessa linha (nunca o do produto base).
  - **price**: **catalog_items.price** dessa linha.
  - **url**: **catalog_items.url** dessa linha (com `#sku=`).
- Se previamente listaste variantes e o utilizador escolhe uma → **fixa a mesma linha** (ref/preço/link já apresentados).
- Se o utilizador fornece um **SKU** → usa exatamente essa linha (ref/preço/link).
...
7) AUTO-CHECK ANTES DE RESPONDER (checklist curto)
- O **SKU/ref mostrado** é **exatamente** o da linha selecionada (não o do produto base)?
- O **preço mostrado** é **exatamente** o `price` dessa linha?
- O **link** é o correto (preferir `…#sku=REF` quando há variante)?
- A resposta inclui “**preço com IVA incluído; portes não incluídos**” (salvo exceção explícita)?
...
def _parse_price(s: str):
    ...
    # remover tudo que não seja dígito, vírgula, ponto ou sinal
    s = re.sub(r"[^\d,.\-]", "", s)
    # caso PT: milhar com ponto e decimal com vírgula (ex.: 1.739,00)
    if "." in s and "," in s and s.rfind(",") > s.rfind("."):
        s = s.replace(".", "").replace(",", ".")
    # caso só vírgula (ex.: 79,00)
...
def _html_to_text(s: str) -> str:
    """
    Converte HTML em texto limpo mantendo quebras razoáveis.
    """
    if not s:
        return ""
    txt = s
...
def _split_attr_segments(raw: str):
    """
    Divide o texto bruto de atributos em segmentos 'chave=valor' (quando possível).
    Regras:
      - divide por vírgula de nível 1 (CSV export costuma separar atributos por vírgulas)
      - remove prefixos como [CS], [RB], etc.
      - corta tudo após ':' (normalmente URL de imagem)
    Retorna lista de tuplos (key, value) ou (None, token) se não houver '='.
    """
    if not raw:
        return []
    ...
    out = []
    for p in parts:
        p = re.sub(r"^\[[^\]]+\]\s*", "", p)  # remove [XX] no início
...
def _variant_to_values_only(raw: str) -> str:
    """
    De 'Cor=Soft Indigo, Cabo=Tecido Areia 1m' -> 'Soft Indigo, Tecido Areia 1m'
    De '[CS]Cor=Soft Indigo:https://...'       -> 'Soft Indigo'
    De 'Soft Indigo'                           -> 'Soft Indigo'
    """
    pairs = _split_attr_segments(raw)
    values = [(v if v else (k or "")).strip() for (k, v) in pairs]
    # filtrar vazios e colapsar múltiplos espaços
...
async def catalog_import_csv(file: UploadFile = File(...),
    ...
    Regras:
      ...
      - summary: HTML -> texto limpo; injeta 'Disponibilidade: ...' (apenas no produto base)
      - variant_attrs: NORMALIZADO para **VALORES** (ex.: 'Soft Indigo, Tecido Areia 1m')
      - sem imagens/currency/iva (assume EUR e IVA incluído)
    Campos guardados: name, summary, url, source, ref, price, brand, variant_attrs (VALORES)
    """
    ...
    for row in reader:
        try:
            ...
            # helper: injeta disponibilidade APENAS no produto base
            ...
            if row_type == "Product":
                current_product = {
                    "name": name_raw,
                    "summary": _with_availability_base(summary_clean),  # já inclui disponibilidade
                    "ref": ref or None,
                    "price": price,
                    "brand": brand,
                    "url": base_url,
                ...
                _upsert_catalog_row(
                    ...
                    source="csv",
                    ref=current_product["ref"],      # pode vir vazio (produto base)
                    ...
                    variant_attrs=None               # produto base não tem attrs
                )
            ...
            # ----- variante (SKU) -----
                ...
                # Normalizar para **VALORES** (ex.: "Soft Indigo, Tecido Areia 1m")
                variant_values = _variant_to_values_only(name_raw)

                # summary da variante = summary LIMPO do produto base (já com disponibilidade)
                summary_v = current_product["summary"]
                if variant_values:
                    summary_v = (summary_v + ("\n" if summary_v else "") + f"Variante: {variant_values}").strip()

                # preço: usa o da SKU se houver; senão herda do produto base
                ...
                _upsert_catalog_row(
                    ...
                    material=None,
                    image_url=None,                   # não guardamos imagem
                    variant_attrs=(variant_values or None)  # guardamos **apenas valores**
                )
                items_out.append({
                    "ok": True,
                    "type": "variant",
